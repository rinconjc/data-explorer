       ...
       [c/modal-body
        ...
        [c/modal-footer
         [c/button {:bsStyle "primary"
                    :on-click #(dispatch [:save-query tab-id @query
                                             (fn [] (reset! show? false))]) }
          "Save"]
...
(defn sql-panel [id]
  (let [cm (atom nil)
        ...
        save-fn #(if (nil? @query)
                   (open-modal [query-form id (atom {:sql (.getValue @cm)})])
                   (dispatch [:save-query id (.getValue @cm)]))
    ...
    (fn[id]
      [:div.panel.panel-default.full-height {:style {:padding "0px" :margin "0px" :height "100%"}}
       ...
       [:div.panel-body {:style {:padding "0px" :overflow "scroll" :height "calc(100% - 46px)"}}
        [code-mirror cm {:mode "text/x-sql" :extraKeys {}}
         (or (get @query :sql) "--...")]]])))
